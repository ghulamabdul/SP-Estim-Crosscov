#######################################################################
###################### Part 3c of the Data application code ############
#######################################################################

###################################################################################################
######### In this part, we estimate Semi parametric model with Delta 2 (for i=77:100)     ###########
...
sp2esti<-foreach(i=77:100)%dopar%{
  library(fields)
  freq.max<-9
  n.nodes<-500
  u<-seq(0,freq.max,length=n.nodes)
  ...
  mle_sp<-function(p,z,dmat.ml,Bes_mat.ml,index.mat.ml,index.val.ml,u.ml,uniq.dist.ml)
  {
    a1<-p[1]
    nu1<-p[2]
    sigma1<-p[3]
  ...
  {
    return(mle_sp(p=pars,z=c(un.grd.train$PM2_5,un.grd.train$WS),dmat.ml=dist.mat.train,Bes_mat.ml=full.bessel[,bessel.index[[i]]],index.mat.ml=index.mat.train.f.list[[i]],index.val.ml=index.val.train.f.list[[i]],u.ml=u,uniq.dist.ml=uniq.dist.train)$mlv)
  }
  ...
  init.value<-optim(par = c(0,0,0,0,0,0,0,0),l2dist,
                    hessian=FALSE,
                    control=list(trace=6,
                                 maxit=10000))
  
  init.spd2<-c(o_candi.estims[[i]]$bvm$par[1:6],init.value$par,o_candi.estims[[i]]$bvm$par[10:11])
  
  
   
  optim_spd2_loglik <- function(par){
    optim(par=par,
          fn = mle_sp_mlv,
          hessian=FALSE,
          control=list(trace=6,
  ...
  spd2.estim<-optim_spd2_loglik(init.spd2)
  
  spd2.estim
  
}
rm(full.bessel,full.theta)
save.image("p3c.RData")
