...
#####################################################################
setwd("/ibex/scratch/qadirga/Simulation_Studies_Old/Increasing_coherence_function")
...
#tr.sp<-c(0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5)
...
optim.pars<-foreach(i=1:100) %dopar%
{
  ...
  loglik_Model.delta.1.allcomponent <- function(theta,locs,data.at.locs,cmats) {
    ...
    if(a1<=0||sigmasq1<=0||a2<=0||sigmasq2<=0||b_3<=-1||b_2<=-1||b_1<=-1||b0<=-1||b1<=-1||b2<=-1||b3<=-1||b4<=-1||b_3>=1||b_2>=1||b_1>=1||b0>=1||b1>=1||b2>=1||b3>=1||b4>=1)
    {
      ...
      u<-seq(0,freq.max,length=length(uniq.dist)-1)
      f.var1<-f.matern(w=u, nu=1, sigmasq = 1, a=a1, d=2)  ####### Spectral density for variable 1
      f.var2<-f.matern(w=u, nu=1, sigmasq = 1, a=a2, d=2)  ####### Spectral density for variable 2
      delta.s<-1
  ...
  init.2<-c(0.8,0.8,0.8,0.8,tr.sp)
  optim(par=init.2,fn=loglik_Model.delta.1,control=list(maxit=6000))
}

save.image("estimatedinc4.RData")
par(mfrow=c(1,2))
...
lines(u,avg.coh+1.96*sd.coh,col="blue",lty=3)
lines(u,avg.coh-1.96*sd.coh,col="blue",lty=3)
...
save.image("Fullanalysisinc4.RData")
